...
void glview::view2d::post_create() {
  ...
  surface = SDL_CreateRGBSurface(0, gl::w(), gl::h(), 32, 0xFF000000,
                                 0x00FF0000, 0x0000FF00, 0x000000FF);
  auto debug_font = (font_dir / "SourceCodePro-Regular.ttf").string();
  con::log_all(con::severity::debug, std::string{"with font "} + debug_font);
...
void glview::view2d::render() {
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         ui_texture, 0);
  glBlitNamedFramebuffer(framebuffer, 0, 0, gl::h(), gl::w(), 0, 0, 0, gl::w(),
                         gl::h(), GL_COLOR_BUFFER_BIT, GL_NEAREST);
  t1 = SDL_GetTicks();
  last_ms = t1 - t0;
